// Phong fragment shader phongEnvMap.frag 
#version 330
...
void main(void) {
	...
	vec4 tmp_Color = (diffuseI + specularI)*texture(texMap, ex_TexCoord)*texture(cubeMap, reflectTexCoord);

	//Attenuation does not affect transparency
	vec4 litColour = vec4(tmp_Color.rgb *attenuation, tmp_Color.a);
	vec4 amb=min(ambientI*texture(texMap, ex_TexCoord)*texture(cubeMap, reflectTexCoord),vec4(1.0f));
		
	out_Color= amb + litColour;
